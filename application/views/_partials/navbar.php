...
if($login->type === 'user'){
    $employee = $this->db->select('employee .*, job_title.job_title, department.department, ')
        ...
        ->where('employee.id', $login->employee_id)
        ->get()->row();
    $mail = $this->db->get_where('inbox', array(
        'to_emp_id' => $employee->id,
        'to_type' => 'employee',
        'reading' => 0,
    ))->result();
}else{
                        ...
                        <li>
                            ...
                            <ul class="menu">
                                <?php if($totalMail) { foreach($mail as $item){ ?>
                                    ...
                                    <li><!-- start message -->
                                        <a href="<?php echo base_url('employee/mail/viewEmail/').'/'.str_replace(array('+', '/', '='), array('-', '_', '~'), $this->encrypt->encode($item->id)) ?>">
