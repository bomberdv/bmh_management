...
class Crud_Model  extends MY_Model
{
    ...
    function _callback_action_order_process($value, $row){
        ...
        return "<a class='btn btn-default move-processing' href='" . base_url()."admin/sales/move_for_shipping/" . get_orderID($orderId) . "' ><i class='fa fa-arrow-right' ></i>". lang('move_to_shipping')."</a>";
    }

    function _callback_action_order_shipment($value, $row){
        $orderId = strip_tags($row->id);
        return "
        <a class='btn btn-default move-shipping' href='" . base_url()."admin/sales/marked_shipped/" . get_orderID($orderId) . "' ><i class='fa fa-truck' ></i>". lang('mark_shipped')."</a>";
    }
    ...
    function _callback_action_label_print($value, $row){
        ...
        return "
            <div class=\"btn-group\" role=\"group\" aria-label=\"...\">
              <a href='" . base_url()."admin/sales/packing_list/" . get_orderID($orderId) . "' type=\"button\" class=\"btn btn-success\">". lang('packing_list')."</a> 
            </div>";
    }
            ...
            return '
                        <div class="btn-group dropdown">
                            ...
                            <ul class="dropdown-menu pull-right">
                                <li> <a href="sales/sale_preview/' . get_orderID($orderId) . '"><i class="fa fa-eye text-success"></i>'. lang('view').'</a> </li>
                                <li> <a href="sales/updateSales/' .get_orderID($orderId) . '"><i class="fa fa-shopping-cart text-success"></i>'. lang('edit').'</a></li>
                                <li> <a href="sales/createPdfInvoice/' . get_orderID($orderId) . '"><i class="fa fa-file-pdf-o text-success"></i>'. lang('pdf_quotation').'</a> </li>
                                <li> <a href="sales/sendInvoice/' . get_orderID($orderId) . '" data-target="#modalSmall" data-toggle="modal"><i class="fa fa-envelope-o text-success" ></i>'. lang('email_quotation').'</a> </li>
                                
                                <li>  <a  data-target="#modalSmall" data-toggle="modal" href="sales/cancelQuotation/' . get_orderID($orderId) . '">
                                        <i class="fa fa-times-circle-o text-danger"></i>'. lang('cancel_quotation').' </a> </li>
                                <li><a onclick="return confirm(\'Are you sure want to delete this Invoice ?\');" href="sales/deleteInvoice/' . get_orderID($orderId) . '"><i class="fa fa-trash-o text-danger"></i>'. lang('delete').'</a> </li>
                            </ul>
                        </div>
                    ';
        }else{
            return '
                        <div class="btn-group dropdown">
                            ...
                            <ul class="dropdown-menu pull-right">
                                ...
                                <li> <a href="sales/sale_preview/' . get_orderID($orderId) . '"><i class="fa fa-eye text-success"></i>'. lang('view').'</a> </li>
                                <li> <a href="sales/updateSales/' . get_orderID($orderId) . '"><i class="fa fa-shopping-cart text-success"></i>'. lang('edit').'</a></li>
                                <li> <a href="sales/addPayment/' . get_orderID($orderId) . '" data-target="#modalSmall" data-toggle="modal"><i class="fa fa-money text-success"></i>'. lang('add_payment').'</a> </li>
                                <li> <a href="sales/paymentList/' . get_orderID($orderId) . '" data-target="#myModal" data-toggle="modal"><i class="fa fa-money text-success"></i>'. lang('view_payment').'</a> </li>
                                <li> <a href="sales/createPdfInvoice/' . get_orderID($orderId) . '"><i class="fa fa-file-pdf-o text-success"></i>'. lang('pdf_invoice').'</a> </li>
                                <li> <a href="sales/sendInvoice/' . get_orderID($orderId) . '" data-target="#modalSmall" data-toggle="modal"><i class="fa fa-envelope-o text-success" ></i>'. lang('email_invoice').'</a> </li>
                                <li>  <a  data-target="#modalSmall" data-toggle="modal" href="sales/cancelSales/' . get_orderID($orderId) . '">
    ...
    function _callback_action_reimbursement($value, $row)
    {
        return '
            <div class="btn-group dropdown">
                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            ...
                            <ul class="dropdown-menu pull-right">
                                <li> <a href="employee/reimbursementView/' . my_encode($row->id) . '"><i class="fa fa-eye text-success"></i>'. lang('view').'</a> </li>
                                <li> <a href="employee/reimbursementForm/' . my_encode($row->id) . '"><i class="fa fa-pencil-square-o text-success"></i>'. lang('edit').'</a> </li>
                               
                                <li><a onclick="return confirm(\'Are you sure want to delete this Invoice ?\');" href="employee/reimbursementDelete/' . my_encode($row->id) . '"><i class="fa fa-trash-o text-danger"></i>'. lang('delete').'</a> </li>
                            </ul>
                        </div>
        ';
    }

    function _callback_action_project($value, $row)
    {
        return '
            <div class="btn-group dropdown">
                            <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-expanded="false">
                            ...
                            <ul class="dropdown-menu pull-right">
                                <li> <a href="project/viewProject/summary/' . my_encode($row->id) . '"><i class="fa fa-eye text-success"></i>'. lang('view').'</a> </li>
                                <li> <a href="project/newProject/' . my_encode($row->id) . '"><i class="fa fa-pencil-square-o text-success"></i>'. lang('edit').'</a> </li>
                               
                                <li><a onclick="return confirm(\'Are you sure want to delete this Invoice ?\');" href="project/deleteProject/' . my_encode($row->id) . '"><i class="fa fa-trash-o text-danger"></i>'. lang('delete').'</a> </li>
                            </ul>
                        </div>
        ';
    }

    function _callback_action_approved_manager($value, $row)
    {
        if($row->approved_manager == 'Pending'){
            return '<small class="label bg-yellow">Pending</small>';
        }elseif($row->approved_manager == 'Reject'){
            return '<small class="label bg-red">Reject</small>';
        }else{
            return '<small class="label bg-success">Approved</small>';
        }
    }

    function _callback_action_approved_admin($value, $row)
    {
        if($row->approved_admin == 'Pending'){
            return '<small class="label bg-yellow">Pending</small>';
        }elseif($row->approved_admin == 'Reject'){
            return '<small class="label bg-red">Reject</small>';
        }else{
            return '<small class="label bg-success">Approved</small>';
        }
