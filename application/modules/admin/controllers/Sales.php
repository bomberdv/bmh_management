...
class Sales extends Admin_Controller
{
    ...
    function allOrder()
    {
        ...
        $crud->columns('date','order_id','customer_name','due_date','grand_total','amount_received', 'due_payment','delivery_status', 'actions');
        $crud->order_by('id','desc');
        $crud->where('type','Invoice');
        $crud->order_by('id','desc');

        $crud->display_as('date', lang('date').'(Y-M-D)');
        $crud->display_as('order_id', lang('order_no'));
        ...
        $crud->callback_column('order_id',array($this->crud,'_callback_action_orderNo'));
    ...
    {
        $crud = new grocery_CRUD();
        $crud->columns('date','order_id','customer_name','due_date','due_payment','grand_total', 'actions');
    ...
    {
        $crud = new grocery_CRUD();
        $crud->columns('date','order_id','customer_name','due_date','due_payment','grand_total','actions', 'delivery_status');
    ...
    {
        $crud = new grocery_CRUD();
        $crud->columns('date','order_id','customer_name','tracking','delivery_person','delivery_date');
    ...
    {
        $crud = new grocery_CRUD();
        $crud->columns('date','order_id','customer_name','due_date','grand_total','status', 'actions');
        $crud->order_by('id','desc');
    ...
    function save_sales()
    {
        ...
        if(empty($order_id)){
            ...
            $db_data['order_id'] = INVOICE_PRE + $order_id;
            $this->db->where('id', $order_id);
            $this->db->update('sales_order', $db_data);
        }else{
            //update
            $this->db->where('id', $order_id);
            $this->db->update('sales_order', $data);
    ...
    function add_to_cart()
    {
        ...
        if(count($product)){
            ...
            if($product->type === 'Bundle'){
                ...
                $tax_rate = $this->db->get_where('tax', array('id' => $product->tax_id ))->row()->rate;
                $data = array(
                    'id'                => $id,
                    'qty'               => 1,
                    'price'             => $price,
