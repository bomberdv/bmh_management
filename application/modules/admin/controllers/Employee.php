...
//            $data['deposit'] = $this->db->get_where('users', array('employee_id' => $id))->row();
    ...
    public function save_salary()
    {
        ...
        if($type == 'Hourly'){
            ...
            $this->form_validation->set_rules('hourly_salary', lang('hourly_salary'), 'trim|required|xss_clean');


            if ($this->form_validation->run() == TRUE) {
                $data['hourly_salary'] = (double)$this->input->post('hourly_salary');
                if(!empty($salary_id))
                {
                    //update data
                ...
                $this->message->save_success('admin/employee/employeeDetails?tab=salary/'. my_encode($id) );
            } else {
                $error = validation_errors();;
                $this->message->custom_error_msg('admin/employee/employeeDetails?tab=salary/'. my_encode($id) ,$error);
            }
        ...
        if ($this->form_validation->run()== TRUE) {//monthly salary validation
                ...
                foreach($records as $record)
                {
                    if($record->id == $earning_id[$i] )
                    {
                        ...
                        if($record->value_type == 2 ) //percentage
                        {
                            ...
                            if($record->total_payable == 1) //total payable
                            {
                                $total_payable += ($basic_salary * $_POST[$earning_id[$i]])/100 ;
                            }
                            if($record->cost_company == 1 ) //cost to company
                            {
                                $total_cost_company += ($basic_salary * $_POST[$earning_id[$i]])/100 ;
                            }
...
//            exit();
    ...
    function create_user()
    {
        ...
        if ($this->form_validation->run()== TRUE) {
            ...
            $this->ion_auth_model->tables = array(
                ...
                'users_branch'		=> 'admin_users_branch',
                'login_attempts'	=> 'login_attempts',
            );

            // proceed to create user
            $user_id = $this->ion_auth->register($identity, $password,$email ,$additional_data, $groups, $branchs);
            if ($user_id)
            {
                // success
    ...
    public function reimbursementForm($id = null)
    {
        ...
        if ($form->validate())
        {
            $employee = $this->db->get_where('employee', array('id' => $this->input->post('employee_id')))->row();
            $manager_id = $this->db->get_where('supervisor', array('employee_id' => $employee->id))->row()->supervisor_id;
            if(!empty($this->input->post('approved_admin'))){
                $approved_admin = $this->input->post('approved_admin');
            }else{
                $approved_admin = 'Pending';
            }
            $data= array(
            ...
            $this->message->save_success('admin/employee/reimbursement');

        }

        $this->mViewData['reimbursement'] = $this->db->get_where('reimbursement', array('id' => $id))->row();
        if(!empty($this->mViewData['reimbursement'])){
            $this->mViewData['employee'] = $this->db->get_where('employee', array('department' => $this->mViewData['reimbursement']->department_id))->result();
        }

        $this->mViewData['all_department'] = $this->db->get('department')->result();
        $this->mViewData['manager'] = $this->db->get('employee')->result();
        $this->mViewData['department_id'] = $this->input->post('department_id', TRUE);
        $this->mTitle= lang('new_reimbursement_form');
        $this->mViewData['form'] = $form;
        $this->render('employee/reimbursement_form');
    }

    public function reimbursement()
    {
        $crud = new grocery_CRUD();
        $crud->columns('date','employee_name','department_id','amount','memo', 'approved_manager', 'approved_admin', 'actions');
        $crud->order_by('id','desc');
        $crud->set_relation('department_id', 'department', 'department');

        $crud->display_as('date', lang('date').'(Y-M-D)');
        $crud->display_as('employee_name', lang('employee'));
        $crud->display_as('department_id', lang('department'));
        $crud->display_as('manager_name', lang('manager'));
        $crud->display_as('amount', lang('amount'));
        $crud->display_as('memo', lang('description'));
        $crud->display_as('approved_manager', lang('approved_by_manager'));
        $crud->display_as('approved_by_admin', lang('approved_admin'));
        $crud->display_as('actions', lang('actions'));

        $crud->set_table('reimbursement');

        $crud->callback_column('approved_manager',array($this->crud,'_callback_action_approved_manager'));
        $crud->callback_column('approved_admin',array($this->crud,'_callback_action_approved_admin'));
        $crud->callback_column('actions',array($this->crud,'_callback_action_reimbursement'));
        $crud->unset_add();
        $crud->unset_edit();
        $crud->unset_delete();
        $crud->unset_read();
        ...
        $this->mViewData['crud'] = $crud->render();
        $this->mViewData['title'] = lang('reimbursement_list');

        $this->mTitle .= lang('process_order');
        $this->render('employee/reimbursement_crud');
    }

    function reimbursementView($id=null){
        $id = my_decode($id);
        $id == TRUE || admin_home();
        ...
        $this->render('employee/reimbursement_view');
    }

    function reimbursementDelete($id = null)
    {
        $id = my_decode($id);
        $id == TRUE || admin_home();
        //delete
        $this->db->delete('reimbursement', array('id' => $id));
