...
//	}
	...
	public function  admin_user_create($id = null)
	{
		...
		if ($form->validate())
		{
			...
			// create user (default group as "members")
			$user = $this->ion_auth->register($username, $password, $email, $additional_data, $groups, $branchs);
			if ($user)
			{
				// success
				$messages = $this->ion_auth->messages();
		...
		$groups = $this->ion_auth->groups()->result();
		$branchs = $this->ion_auth->branch()->result();
		$this->mViewData['groups'] = $groups;
		$this->mViewData['branchs'] = $branchs;
	...
	{	
		...
		$select = array(
					'admin_groups.description group_name',
					...
					'email',
					'admin_branch.description branch_name',
		);
	
		$this->mViewData['employees']           = $this->db->select($select)
				->from('admin_users')
				->join('admin_users_groups', 'admin_users.id = admin_users_groups.user_id', 'left')
				->join('admin_users_branch', 'admin_users.id = admin_users_branch.user_id', 'left')
				->join('admin_groups', 'admin_users_groups.group_id = admin_groups.id', 'left')
				->join('admin_branch', 'admin_users_branch.branch_id = admin_branch.id', 'left')
				->get()
				->result();
	...
	function update_profile($id = null){
		...
		$this->mViewData['form'] = $form;
		$this->mViewData['admin_user'] = $this->db->select('admin_users.*, admin_users_groups.id as admin_users_groups_id, admin_groups.id as admin_groups_id, admin_branch.id as admin_branch_id')
				->from('admin_users')
				->join('admin_users_groups', 'admin_users.id = admin_users_groups.user_id', 'left')
				->join('admin_users_branch', 'admin_users.id = admin_users_branch.user_id', 'left')
				->join('admin_groups', 'admin_users_groups.group_id = admin_groups.id', 'left')
		...
		$this->mViewData['branchs'] = $this->ion_auth->branch()->result();
	...
	function account_update_profile()
	{
		...
		else
		{
			...
			$sbdata['branch_id']		= $this->input->post('branchs');

			$this->db->where('user_id', $id);
			$this->db->update('admin_users_groups', $sdata);
			
			$this->db->where('user_id', $id);
			$this->db->update('admin_users_branch', $sbdata);
