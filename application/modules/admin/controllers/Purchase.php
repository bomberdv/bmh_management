...
class Purchase extends Admin_Controller
{
    ...
    function save_purchase()
    {
        ...
        $db_data['order_id'] = INVOICE_PRE + $purchase_id;

        $this->db->where('id', $purchase_id);
        $this->db->update('purchase_order', $db_data);
        ...
        redirect('admin/purchase/purchaseInvoice/'.get_orderID($purchase_id));
    ...
    function purchaseList()
    {
        ...
        $crud->columns('date','order_id','vendor_name', 'purchase_status','grand_total','paid_amount','due_payment', 'actions');
