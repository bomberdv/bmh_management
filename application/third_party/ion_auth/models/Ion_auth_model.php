...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($identity, $password, $email, $additional_data = array(), $groups = array(), $branchs = array())
	{
		...
		elseif ( !$this->config->item('default_branch', 'ion_auth') && empty($branchs) )
		{
			$this->set_error('account_creation_missing_default_branch');
			return FALSE;
		}
		...
		// Check if exist master branch
		$query_branch = $this->db->get_where($this->tables['branch'],array('branch_name' => $this->config->item('default_branch', 'ion_auth')),1)->row();
		if( !isset($query_branch->id) && empty($branchs) )
		{
			$this->set_error('account_creation_invalid_default_branch');
			return FALSE;
		}
		
		// capture default group details
		$default_group = $query;
		$default_branch = $query_branch;
		...
		if( isset($default_branch->id) && empty($branchs) )
		{
			$branchs[] = $default_branch->id;
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		/* $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, type')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']); */
		...
		$this->db->select('a.'. $this->identity_column . ', a.email, a.id, a.password, a.active, a.last_login, a.type, b.branch_id');
		$this->db->from($this->tables['users'].' a');
		$this->db->where('a.'.$this->identity_column, $identity);
		$this->db->join('admin_users_branch b', 'a.id = b.user_id', 'left');
	 ...
	 * @author Ben Edmunds
	 **/
	public function add_to_branch($branch_ids, $user_id=false)
	{
		$this->trigger_events('add_to_branch');
		...
		{
			if ($this->db->insert($this->tables['users_branch'], array( $this->join['branch'] => (float)$this->session->userdata('branch_id'), $this->join['users'] => (float)$user_id)))
			{
				if (isset($this->_cache_groups[$branch_id])) {
					$group_name = $this->_cache_groups[$branch_id];
				}
				else {
					$branch = $this->branchs($branch_id)->result();
					$group_name = $branch[0]->branch_name;
					$this->_cache_groups[$branch_id] = $group_name;
				}
				$this->_cache_user_in_group[$user_id][$branch_id] = $group_name;
	...
	public function branch()
	{
		...
		$this->response = $this->db->get('admin_branch');
	...
	{
		$this->trigger_events('branchs');

		if (isset($id))
		{
			$this->where($this->tables['branch'].'.id', $id);
		}
